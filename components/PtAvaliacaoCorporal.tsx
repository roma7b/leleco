...
const PtAvaliacaoCorporal: React.FC<PtAvaliacaoCorporalProps> = ({ students }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        height: '',   // Altura (cm)
        imc: '',      // IMC (Calculado)
        gender: 'male', // NOVO: Gênero para fórmulas
    ...
    // EFEITO 3: Cálculo Automático da % Gordura via Dobras (Pollock 7-Site)
    const calculateBodyFat = (folds: typeof formData.skinFolds, gender: string, age: number) => {
        if (formData.fatCalculationMethod !== 'Dobras') return null;

        const foldValues = Object.values(folds).map(v => parseFloat(v || '0'));
        if (foldValues.some(v => v <= 0)) return null;

        const sumOfFolds = foldValues.reduce((sum, val) => sum + val, 0);

        // 1. Cálculo da Densidade Corporal (DB) - Fórmulas de Jackson & Pollock (7-Site)
        // Usaremos uma fórmula simplificada (pode ser refinada mais tarde)
        let BD: number;
        ...
        // 2. Conversão da Densidade Corporal para % Gordura (Fórmula de Siri)
        const percentFat = ((4.95 / BD) - 4.50) * 100;

        return isNaN(percentFat) ? null : percentFat.toFixed(2);
    };

    useEffect(() => {
        const ageVal = parseFloat(formData.age);

        if (formData.fatCalculationMethod === 'Dobras' && ageVal > 0) {
    ...
    // Carregar uma avaliação antiga no formulário (Atualizada para Dobras)
    const handleLoadAssessment = (assessment: Assessment) => {
        setFormData({
            ...
            gender: 'male', // Idealmente viria do Student object
            ...
            // Carregamento dos dados de Dobras (assumindo que o banco já tem sf_ prefix)
            skinFolds: { 
    ...
    const handleGenerateAI = async () => {
        // ... (Lógica de geração de IA não alterada, apenas passando os dados do form)
        if (!formData.weight || !formData.bodyFat || !formData.muscleMass) {
            showToast('Preencha pelo menos Peso, Gordura e Músculo.', 'error');
            return;
        }
        setGeneratingAI(true);
        const dadosAtuais = { data: new Date().toLocaleDateString('pt-BR'), ...formData };
    ...
    const renderStrategicReport = () => {
        ...
        // ... (Corpo do renderStrategicReport não alterado)
        return (
            <div className="space-y-6 animate-fadeIn">
                {/* ... (código dos cards de risco, metodologia, destaque, plano tático) ... */}
